...
class FrameTransform(Node):
    def __init__(self):
        ...
        self.tf1_broadcaster = TransformBroadcaster(self) #! From the ros2 example
        self.tf2_broadcaster = TransformBroadcaster(self) #! From the ros2 example
        self.imu_pub = self.create_publisher(Imu, "Imu_data" ,10)

        self.cmd_vel_sub = self.create_subscription(Twist, "cmd_vel", self.cmd_vel_callback, 10)

        self.timer_odom = self.create_timer(0.5, self.odom_timer_callback)
        self.timer_est = self.create_timer(0.2,self.update_car_state)
        ...
        self.sigma_gain = 0.256/0.5
        self.sigma_beta = self.sigma_gain*(1 - self.sigma_alpha)
        ...
        self.vel_gain = 2.0/1.65
        self.vel_beta = self.vel_gain*(1 -self.vel_alpha)
        #!-----------------------------------------------
    ...
    def cmd_vel_callback(self, msg):
        self.sigma_input = msg.angular.z  + self.sigma_offset
        self.vel_input = msg.linear.x

    def update_car_state(self):
            with self.lock:
                # self.sigma[0] = self.sigma_beta * self.sigma_input + (1 - self.sigma_alpha) * self.sigma[1]
                self.sigma[0] = self.sigma_gain*self.sigma_input
                self.vel[0] = self.vel_beta * self.vel_input + self.vel_alpha * self.vel[1]
                ...
                self.yaw_dot = self.vel[0] * math.tan(self.sigma[0])/self.wheel_base
                self.theta += self.yaw_dot * self.dt

                # print(f"X: {self.x:.2f}, Y: {self.y:.2f}, Theta: {self.theta:.2f}, Velocity: {self.vel[0]:.2f}, Sigma: {self.sigma[0]:.2f}")
                # print(f"X: {self.x:.2f}, Theta: {self.theta*180/3.1416:.2f}, Sigma: {self.sigma[0]*180/3.1416:.2f}")
                print(f"X: {self.x:.2f}")
    ...
    def odom_timer_callback(self):
        with self.lock:
            t = TransformStamped()
            times = self.get_clock().now().to_msg()
            q = quaternion_from_euler(0, 0, self.theta)

            # Create odometry message
            odom_msg = Odometry()
            odom_msg.header.stamp = times
            ...
            if (t.transform.translation.x != 0.):
                self.publisher_odom.publish(odom_msg)
                self.tf1_broadcaster.sendTransform(t)
                self.tf2_broadcaster.sendTransform(t2)
                print("published")
